...
class BTS7960: public MotorInterface
{
    private:
        ...
        // In the two-PWM scheme the "pwm" pin is not a PWM pin at all: IN_A and
        // IN_B carry the complementary PWM and the third pin is the driver's
        // enable, held high for as long as the motor exists. A BTS7960 module
        // has R_EN / L_EN; the Waveshare General Driver's TB6612 has its PWMA /
        // PWMB tied to the same role. Doing it here, from the pin the config
        // already names, is what makes `gpio_out: 25=1,26=1` unnecessary --
        // the enable lines are part of the driver, not of the board init.
    ...
    public:
        BTS7960(float pwm_frequency, int pwm_bits, bool invert, int enable_pin, int in_a_pin, int in_b_pin): 
            MotorInterface(invert),
            in_a_pin_(in_a_pin),
            in_b_pin_(in_b_pin)
        {

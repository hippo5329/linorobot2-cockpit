...
The fake-mode Nav2 matrix is unaffected by design: every fake leg has
fake_wheels true, so publish_mag is true, so the topic names and madgwick stay
...
def test_the_choice_is_keyed_on_publish_mag_not_a_new_flag():
    """publish_mag is already (a real magnetometer answered) OR (fake wheels are
    synthesising a field). Keying on it is what keeps every fake-mode leg on

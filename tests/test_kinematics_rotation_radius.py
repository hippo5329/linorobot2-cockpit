...
RPM and the RPM back into odometry, so in simulation mode the error cancels exactly.
...
class Base:
    ...
    def round_trip(self, vx, vy=0.0, wz=0.0):
        """Command -> wheels -> measured motion, the way simulation mode does it."""
...
def test_the_two_halves_cannot_disagree(kin_lib):
    ...
    This is the property simulation mode DOES have, and the reason simulation mode cannot

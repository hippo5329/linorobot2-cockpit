...
`use_fake_*` keys someone remembered and the schema has six -- so `fake_sonar`
...
them, so adding a seventh fake sensor fails here rather than passing quietly.
"""
import os
import re
...
def _schema_fake_keys():
    """Every use_fake_* key the env writer knows how to translate."""
    src = open(MCU_ENV, encoding="utf-8").read()
    return set(re.findall(r"use_fake_[a-z0-9_]+", src))
...
def test_the_schema_has_the_keys_this_test_thinks_it_has():
    ...
    keys = _schema_fake_keys()
    assert len(keys) >= 5, f"only found {keys} -- the enumeration has stopped working"
    assert "use_fake_sonar" in keys, "the key this test exists for is gone"


@pytest.mark.skipif(not os.path.exists(WRITER), reason="the lab repo is not checked out here")
def test_the_real_sensor_writer_disables_every_fake_flag():
    src = open(WRITER, encoding="utf-8").read()
    declared = set(re.findall(r'"(use_fake_[a-z0-9_]+)"', src))
    missing = _schema_fake_keys() - declared
    assert not missing, (
...
def test_the_writer_sets_them_from_the_set_not_one_by_one():
    ...
    here, and nothing says so -- which is exactly how fake_sonar survived."""
    src = open(WRITER, encoding="utf-8").read()
    assert "for key in sorted(FAKE_KEYS)" in src, \
